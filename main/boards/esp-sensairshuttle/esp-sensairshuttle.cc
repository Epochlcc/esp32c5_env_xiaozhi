...
#include "services/bme690_service.h"
#include "sdkconfig.h"
...
public:
    ...
    Cst816d(i2c_master_bus_handle_t i2c_bus, uint8_t addr) : I2cDevice(i2c_bus, addr)
    {
        ...
        was_touched_ = false;
        press_count_ = 0;
        // Verify CST816D communication by reading chip ID (register 0xA7)
        chip_id_ = ReadReg(0xA7);
        ESP_LOGI(TAG, "CST816D chip ID: 0x%02X", chip_id_);
        if (chip_id_ == 0xFF || chip_id_ == 0x00) {
            ESP_LOGW(TAG, "CST816D chip ID looks invalid (0x%02X), touch may be unreliable", chip_id_);
        }
    }
    ...
    bool IsAlive() const
    {
        return chip_id_ != 0xFF && chip_id_ != 0x00;
    }
    ...
    void UpdateTouchPoint()
    {
        ...
        // Clear the buffer so stale data won't persist if next I2C read fails
    ...
    {
        // Reject invalid coordinates outside the 320x240 display area
        // (4095,4095) occurs when I2C data is all 0xFF (bus pull-up noise)
        if (tp_.num > 0 && (tp_.x >= 320 || tp_.y >= 240)) {
            ESP_LOGW(TAG, "Invalid touch coordinates: (%d, %d), ignoring", tp_.x, tp_.y);
            tp_.num = 0;
            was_touched_ = false;
            return TOUCH_NONE;
        }
...
private:
    ...
    void InitializeBme690()
    {
        ...
        if (ret == ESP_OK) {
            ...
            ESP_LOGW(TAG, "BME690 init failed: %s, continuing without environment sensor", esp_err_to_name(ret));
        }
    }
...
public:
    ...
    Bme690Service& GetBme690Service()
    {
        return bme690_service_;
    }
};
