...
void LcdDisplay::InitializeLcdThemes() {
    ...
    auto emoji_collection = std::make_shared<Twemoji64>();
...
void LcdDisplay::CreateEnvironmentPanel() {
    if (env_panel_ != nullptr) {
        return;
    }
...
void LcdDisplay::DestroyEnvironmentPanel() {
    if (env_panel_ != nullptr) {
        lv_obj_del(env_panel_);
        env_panel_ = nullptr;
        env_temp_label_ = nullptr;
        env_humidity_label_ = nullptr;
        env_iaq_label_ = nullptr;
        env_co2_label_ = nullptr;
...
void LcdDisplay::SetEnvironmentMode(bool enabled) {
    if (enabled == env_mode_) {
        return;
    }
    ...
    if (enabled) {
        ...
        lv_obj_remove_flag(env_panel_, LV_OBJ_FLAG_HIDDEN);
    } else {
        if (env_panel_ != nullptr) {
            lv_obj_add_flag(env_panel_, LV_OBJ_FLAG_HIDDEN);
        }
    ...
    env_mode_ = enabled;
    ESP_LOGI(TAG, "Environment mode %s", enabled ? "enabled" : "disabled");
}

void LcdDisplay::UpdateEnvironmentData(float temperature, float humidity, float iaq,
                                        uint8_t iaq_accuracy, float co2_equivalent, float pressure) {
    if (env_temp_label_ == nullptr) {
        return;
    }
