...
void AudioService::Start() {
    ...
    xTaskCreate([](void* arg) {
        ...
        audio_service->AudioInputTask();
        vTaskDelete(NULL);
    }, "audio_input", 2048 * 6, this, 8, &audio_input_task_handle_);
...
void AudioService::AudioInputTask() {
    while (true) {
        ...
        /* Handle deferred power-off from timer callback (safe task context) */
        if (bits & AS_EVENT_POWER_OFF_REQ) {
            xEventGroupClearBits(event_group_, AS_EVENT_POWER_OFF_REQ);
            if (pending_power_off_input_.exchange(false)) {
...
void AudioService::AudioOutputTask() {
    while (true) {
        ...
        audio_queue_cv_.wait(lock, [this]() {
            return !audio_playback_queue_.empty() || service_stopped_ || pending_power_off_output_.load();
        });
        if (service_stopped_) {
            break;
        }
        ...
        /* Handle deferred power-off from timer callback (safe task context) */
        if (pending_power_off_output_.load()) {
            pending_power_off_output_ = false;
            lock.unlock();
...
void AudioService::CheckAndUpdateAudioPowerState() {
    ...
    /* ESP-SenseAirShuttle 使用 ADC 连续模式读取 + I2S PDM TX。在 ESP32-C5 (ESP-IDF 5.5.4) 上
     * 即使用 ESP_TIMER_TASK 分发也会触发 xTaskPriorityDisinherit 断言。
     * 完全跳过省电逻辑以便排查问题。 */
    return;
#endif
    ...
    // Set pending flags and notify audio tasks instead of calling EnableInput/Output directly.
    // Actual codec disable happens in AudioInputTask / AudioOutputTask (safe task context).
    bool power_off_pending = false;

    if (input_elapsed > AUDIO_POWER_TIMEOUT_MS && codec_->input_enabled()) {
        pending_power_off_input_ = true;
    ...
    if (output_elapsed > AUDIO_POWER_TIMEOUT_MS && codec_->output_enabled()) {
        ...
        if (!(codec_->duplex() && codec_->input_enabled())) {
            pending_power_off_output_ = true;
            audio_queue_cv_.notify_all();
            power_off_pending = true;
        }
    }
    ...
    if (power_off_pending) {
        xEventGroupSetBits(event_group_, AS_EVENT_POWER_OFF_REQ);
    } else if (!codec_->input_enabled() && !codec_->output_enabled()) {
        esp_timer_stop(audio_power_timer_);
    }
}
